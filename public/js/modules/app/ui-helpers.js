...
export function extractCode(text) {
	if (!text) return "";
	const keywords =
		"(?:验证码|校验码|激活码|one[-\\s]?time\\s+code|verification\\s+code|security\\s+code|two[-\\s]?factor|2fa|otp|login\\s+code|code)";
	...
	let m = text.match(
		new RegExp(
			keywords +
				"[^0-9A-Za-z]{0,20}(?:is(?:\\s*[:：])?|[:：]|为|是)?[^0-9A-Za-z]{0,10}(\\d{4,8})" +
				notFollowAlnum,
			"i",
		),
	);
	...
	m = text.match(
		new RegExp(
			keywords +
				"[^0-9A-Za-z]{0,20}(?:is(?:\\s*[:：])?|[:：]|为|是)?[^0-9A-Za-z]{0,10}((?:\\d[ \\t-]){3,7}\\d)",
			"i",
		),
	);
	if (m) {
	...
	m = text.match(
		new RegExp(
			keywords +
				"[^0-9A-Za-z]{0,40}((?=[0-9A-Za-z]*\\d)[0-9A-Za-z]{4,8})" +
				notFollowAlnum,
			"i",
		),
	);
