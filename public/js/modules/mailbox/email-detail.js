...
export function renderEmailDetail(email) {
	if (!email) {
		return '<div class="empty-detail">Vui lòng chọn một email</div>';
	}

	const sender = escapeHtml(email.sender || "Người gửi chưa rõ");
	const to = escapeHtml(email.to_addrs || "");
	const subject = escapeHtml(email.subject || "(Không có chủ đề)");
        ...
        <div class="detail-meta">
          ...
          ${
            ...
            <div class="meta-row verification-code">
              <span class="meta-label">Mã xác thực:</span>
              <span class="meta-value code-value" title="Nhấn để sao chép">${escapeHtml(verificationCode)}</span>
            </div>
          `
							: ""
					}
...
export function renderEmailModal(email) {
    ...
    <div class="modal-meta">
      ...
      ${to ? `<p><strong>Người nhận:</strong> ${to}</p>` : ""}
      <p><strong>Thời gian:</strong> ${receivedAt}</p>
      ${
				verificationCode
					? `
        <p class="verification-code">
          <strong>Mã xác thực:</strong>
          <span class="code-value" data-code="${escapeAttr(verificationCode)}" title="Nhấn để sao chép">${escapeHtml(verificationCode)}</span>
        </p>
      `
					: ""
			}
    ...
    <div class="modal-footer">
      <button class="btn btn-danger" data-action="delete" data-email-id="${email.id}">Xóa email</button>
...
export function extractVerificationCode(text) {
	...
	const keywords =
		"(?:验证码|校验码|激活码|verification\\s+code|security\\s+code|otp|code)";
