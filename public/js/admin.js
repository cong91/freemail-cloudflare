...
import {
	...
	resetEditState,
	validateEditForm,
} from "./modules/admin/user-edit.js";
import {
	formatTime,
	...
	renderUserList,
	renderUserRow,
} from "./modules/admin/user-list.js";
...
async function loadUsers() {
	...
	try {
		...
		console.error("Tải người dùng thất bại:", e);
...
async function openEditModal(userId) {
	try {
		...
		showToast("Tải thông tin người dùng thất bại", "error");
	}
}
...
async function loadUserMailboxes() {
	...
	try {
		const data = await getUserMailboxes(currentViewingUser.id, {
			page: mailboxPage,
        ...
        <div class="mailbox-item clickable" data-address="${m.address}" data-href="/?mailbox=${encodeURIComponent(m.address)}">
          ...
          <button class="btn btn-sm danger" data-action="unassign">Hủy gán</button>
        </div>
      `,
						)
						.join("")
				: '<div class="empty">Chưa có hộp thư</div>';
			...
			container.querySelectorAll('[data-action="unassign"]').forEach((btn) => {
				btn.onclick = async (e) => {
					...
					const confirmed = await showConfirm(
						`Bạn có chắc muốn hủy gán hộp thư ${address}?`,
					);
					if (!confirmed) return;

					try {
						await unassignMailbox(currentViewingUser.username, address);
						showToast("Đã hủy gán", "success");
						loadUserMailboxes();
					} catch (e) {
						showToast("Hủy gán thất bại", "error");
					}
				};
			});
		}
		...
		if (els.mailboxesPageInfo)
			els.mailboxesPageInfo.textContent = `${mailboxPage} / ${totalPages}`;
		if (els.mailboxesPrevPage)
			els.mailboxesPrevPage.disabled = mailboxPage <= 1;
		if (els.mailboxesNextPage)
			els.mailboxesNextPage.disabled = mailboxPage >= totalPages;
	} catch (e) {
		showToast("Tải hộp thư thất bại", "error");
	} finally {
...
async function handleAssignMailbox() {
	...
	try {
		...
		if (successCount > 0 && failCount === 0) {
			showToast(`Gán thành công ${successCount} hộp thư`, "success");
		} else if (successCount > 0 && failCount > 0) {
			showToast(`Thành công ${successCount}, thất bại ${failCount}`, "warning");
		} else {
			showToast("Gán thất bại", "error");
		}
...
async function handleUnassignMailbox() {
	...
	try {
		...
		if (successCount > 0 && failCount === 0) {
			showToast(`Hủy gán thành công ${successCount} hộp thư`, "success");
		} else if (successCount > 0 && failCount > 0) {
			showToast(`Thành công ${successCount}, thất bại ${failCount}`, "warning");
		} else {
			showToast("Hủy gán thất bại", "error");
...
els.uClose?.addEventListener("click", () =>
	els.uModal?.classList.remove("show"),
);
els.uCancel?.addEventListener("click", () =>
	els.uModal?.classList.remove("show"),
);
...
els.aClose?.addEventListener("click", () =>
	els.aModal?.classList.remove("show"),
);
els.aCancel?.addEventListener("click", () =>
	els.aModal?.classList.remove("show"),
);
els.aAssign?.addEventListener("click", handleAssignMailbox);

// 取消分配模态框
els.unassignOpen?.addEventListener("click", () =>
	els.unassignModal?.classList.add("show"),
);
els.unassignClose?.addEventListener("click", () =>
	els.unassignModal?.classList.remove("show"),
);
els.unassignCancel?.addEventListener("click", () =>
	els.unassignModal?.classList.remove("show"),
);
els.unassignSubmit?.addEventListener("click", handleUnassignMailbox);

// 编辑模态框
els.editClose?.addEventListener("click", () =>
	els.editModal?.classList.remove("show"),
);
...
els.editDelete?.addEventListener("click", async () => {
	...
	const confirmed = await showConfirm(
		`Bạn có chắc muốn xóa người dùng "${currentViewingUser.username}" không? Thao tác này không thể hoàn tác.`,
	);
	if (!confirmed) return;

	try {
		await deleteUser(currentViewingUser.id);
		showToast("Người dùng đã bị xóa", "success");
		els.editModal?.classList.remove("show");
		loadUsers();
	} catch (e) {
